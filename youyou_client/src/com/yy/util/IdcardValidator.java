...
public class IdcardValidator {
	...
	public static boolean IDCheck(String IDStr){
		if(ValUtil.strNotNull(IDStr)){
			try{
	...
	public static boolean IDCardValidate(String IDStr) throws ParseException {
		...
		int TotalmulAiWi = 0;
		for (int i = 0; i < 17; i++) {
			TotalmulAiWi = TotalmulAiWi + Integer.parseInt(String.valueOf(Ai.charAt(i))) * Wi[i];
		}
		...
		if (IDStr.length() == 18) {
			if (Ai.equals(IDStr) == false) {
				errorInfo = "身份证无效，最后一位字母错误,请输入大写！";
				System.out.println(errorInfo);
				return false;
			}
		} else {
	...
	public static String getBirthByIdCard(String idCard) {
		try {
			...
			if (15 == length) {
				...
				year = (Integer.valueOf(year) > 10 ? ("19" + year) : ("20" + year)) + "-";
	...
	public static int getAgeByIdCard(String idCard) {
		...
		try {
			...
			Date birthDate = sdfshort.parse(birth);
			Date today = new Date();
			int age = (int) ((today.getTime() - birthDate.getTime()) / (1000L * 24 * 3600 * 365));
			return age;
		} catch (Exception e) {
			e.printStackTrace();
			return -1;
	...
	public static String getSexByIdCard(String idCard){
		...
		if(15==idCard.length()){
			int sexNum = Integer.valueOf(idCard.substring(13, 14));
			if(sexNum%2==0){
				return "2";
			}else{
				return "1";
	...
	public static void main(String[] args) throws ParseException {
		System.out.println(IDCardValidate("512219220818631"));
		System.out.println(IDCardValidate("612209220815632"));
	}
