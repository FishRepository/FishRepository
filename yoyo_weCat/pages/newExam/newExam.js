...
Page({
  ...
  onLoad: function (options) {
    ...
    that.setData({
      ...
      key: options.sectionType + "-" + options.postType + "-" + options.certType + "key"
    })
    wx.showLoading({
      title: '正在加载试题...'
    }),
      ...
      wx.getStorage({
        ...
        fail: function () {
          wx.request({
            url: app.globalData.hostUrl + '/admin.do?method=getSectionTopic',
            data: {
      ...
      if (that.data.resData.LIST[index].showExplain == '0') {
        wx.showModal({
          ...
          success: function (res) {
            if (res.confirm) {
              wx.request({
                url: app.globalData.hostUrl + '/admin.do?method=getSectionExplain',
                data: {
                  "topicId": topicId
                },
                method: "post",
